...
public class PancakeEasy {
    ...
    private static int solve(Scanner in) {
        ...
        if(D ==2){
            ...
            Optional<Map.Entry<Long, Long>> minDouble = freq.entrySet().stream().filter(e -> e.getValue() >= 2).min(Comparator.comparing(Map.Entry::getKey));
            if(minDouble.isPresent()){
                if(a.stream().anyMatch(l -> l > minDouble.get().getKey())) return 1;
            }
            for (Long l : a) {
                for (Long l2 : a) {
                    if(l == 2*l2) return 1;
