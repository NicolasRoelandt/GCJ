...
public class ArraySolution {
    ...
    private static long solve(int max){
        long count = 0;
        int min = max-(R+C-2);
        Buckets buckets = new Buckets(max, min);
    ...
    private static class Buckets{
        ...
        public Buckets(int max, int min) {
            this.shift = min+1;
            this.buckets = (Set<Cell>[]) new Set[max-min];;
        }

        private void addTo(int h, Cell cell){
            int index = h - shift;
            if(index < 0) return;
